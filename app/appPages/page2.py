...
def page2():
    ...
    duration_sec = first_recording.get("duration", 0)
    ...
    with st.container(border=False):
        ...
        with r1columns[1]:
            with st.container(border=subBorders, height=metricHeight):
                ...
                st.markdown(f"#### {duration_sec:.1f} sec")
        with r1columns[2]:
            with st.container(border=subBorders, height=metricHeight):
                st.markdown(":small[Split clips:]")
                st.markdown(f"#### {n_recordings}")
        with r1columns[3]:
            with st.container(border=subBorders, height=metricHeight):
                st.markdown(f":small[Total audio length:]")
                st.markdown(f"#### {(duration_sec*n_recordings)/60:.1f} min")
    ...
    clips_per_speaker = [len(audio_data [speaker]) for speaker in speakers]
    
    # RMS distribution for threshold selection
    blank_lines(2)
    st.markdown("#### Analyze RMS Distribution")
    st.write("Examine the energy distribution to choose an appropriate filtering threshold")
    
    rms_fig = create_rms_distribution_chart(audio_data)
    if rms_fig:
        st.plotly_chart(rms_fig)
    
    # Interactive threshold setting
    blank_lines(2)
    st.markdown("#### Set Filtering Threshold and Analyze Impact")
    
    # Calculate reasonable threshold range
    all_rms = []
    for clips in audio_data.values():
    ...
    with cols[0]:
        ...
        threshold_fig = create_threshold_analysis_chart(audio_data, rms_threshold)
        if threshold_fig:
            st.plotly_chart(threshold_fig)
    
    with cols[1]:
        # Calculate and display filtering impact
        impact_df = calculate_filtering_impact(audio_data, rms_threshold)
    ...
    st.info(f"**Balanced dataset will have:** {total_after_balance} clips ({min_kept} per speaker)")
    st.info(f"**Total clips removed:** {total_removed} ({(total_removed/total_clips)*100:.1f}% of original)")
    
    # Sample clip inspection
    blank_lines(2)
    st.markdown("#### Inspect Threshold Boundary Clips")
    st.write("Listen to clips near the threshold to verify filtering quality. Make sure clips marked 'Keep' \
             sound good and clips marked 'Filter' are indeed almost or completely silent.")
    ...
    st.write("Chosen filtering threshold:")
    st.code(f"RMS_THRESHOLD = {rms_threshold:.6f}", language="python")
    st.write("Copy this value into the configuration of the next step, `3-filter-and-balance.py` before \
             running that step to use it.")
...
def create_balance_analysis_chart(impact_df):
    ...
    fig.add_trace(go.Bar(
        ...
        marker_color=[ SPEAKER_COLOURS.get(speaker, '#888888') for speaker in impact_df['Speaker']],
